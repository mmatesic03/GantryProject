...
def map_paths_to_gantry_mm(
    ...
    """
    Map sampled paths from image pixel coordinates to gantry mm coordinates.

    Coordinate system matches the Arduino firmware:
      (0,0) = safe top-left work origin after homing
      +X = right
      +Y = down

    The output is fitted inside the usable gantry work area rather than full A4.
    """
    img_h_px, img_w_px = image_shape  # OpenCV shape is (height, width)
    ...
    # Uniform scaling only, so the image is not distorted.
    ...
    for path_px in paths_px:
        if len(path_px) == 0:
            mapped_paths_mm.append(path_px.copy())
            continue
        ...
        # Do NOT flip Y. Arduino uses top-left origin and +Y downward.
        x_mm = x_px * s + x_offset_mm
        y_mm = y_px * s + y_offset_mm
...
def save_gantry_preview(
    ...
    """
    Save a visual preview of sampled paths in gantry coordinates.
    ...
    # Work area border
    work_x = [0, work_w_mm, work_w_mm, 0, 0]
    work_y = [0, 0, work_h_mm, work_h_mm, 0]
    ax.plot(work_x, work_y, linewidth=1.5, label="Gantry work area")

    # Margin / drawable area border
    draw_x0, draw_y0 = margin_mm, margin_mm
    draw_x1, draw_y1 = work_w_mm - margin_mm, work_h_mm - margin_mm
    ...
    ax.set_xlim(-5, work_w_mm + 5)
    ax.set_ylim(work_h_mm + 5, -5)  # invert Y so top-left origin is displayed correctly
    ax.set_aspect("equal", adjustable="box")
    ax.set_xlabel("X (mm)")
    ax.set_ylabel("Y (mm, down)")
    ax.set_title("Gantry Path Preview (Top-left origin)")
...
def paths_to_arduino_commands(paths_mm: list[np.ndarray]) -> list[tuple[float, float, int]]:
    """
    Convert mapped gantry paths into Arduino commands.
    ...
    for path in paths_mm:
        ...
        # Travel to start of this contour with pen up.
        commands.append((float(path[0, 0]), float(path[0, 1]), 0))

        # Draw the contour.
        for p in path[1:]:
            commands.append((float(p[0]), float(p[1]), 1))

    return commands


def save_arduino_commands(
    commands: list[tuple[float, float, int]],
    output_path: Path,
) -> None:
    """Save commands in the exact serial format Arduino will receive."""
    with open(output_path, "w", encoding="utf-8") as f:
        f.write("# x_mm,y_mm,mode\n")
        f.write("# mode 0 = travel / pen up\n")
        f.write("# mode 1 = draw / pen down\n")
        for x, y, mode in commands:
            f.write(f"{x:.2f},{y:.2f},{mode}\n")


def validate_arduino_commands(
    commands: list[tuple[float, float, int]],
    work_w_mm: float,
    work_h_mm: float,
) -> None:
    """Fail before sending if any command is outside the gantry work area."""
    for i, (x, y, mode) in enumerate(commands):
        ...
        if not (0.0 <= x <= work_w_mm and 0.0 <= y <= work_h_mm):
            raise ValueError(
                f"Command {i} outside gantry bounds: "
                f"x={x:.2f}, y={y:.2f}, allowed X=0..{work_w_mm}, Y=0..{work_h_mm}"
            )
...
def send_commands_to_arduino(
    ...
    """
    Stream coordinates to Arduino over USB serial.

    Arduino firmware should:
      - home on startup
      - print READY when ready
      - accept lines formatted as x,y,mode
      - print OK after finishing each move
      - accept END when finished
    """
    if serial is None:
    ...
    with serial.Serial(port, baud, timeout=2) as arduino:
        # Uno usually resets when the serial port opens.
        time.sleep(2)
        ...
        for i, (x, y, mode) in enumerate(commands):
            message = f"{x:.2f},{y:.2f},{mode}\n"
            print(f"Sending {i + 1}/{len(commands)}: {message.strip()}")
            ...
            while True:
                ...
                if response == "OK":
                    break
                if response.startswith("ERROR") or response.startswith("EMERGENCY"):
...
def main():
    ...
    image_path = repo_root / "input_images" / "cats.jpg"
    ...
    # Gantry work area used by the Arduino firmware.
    # Your current horizontal axis is limited, so do not output full A4 width.
    work_w_mm = 150.0
    work_h_mm = 270.0
    margin_mm = 5.0

    # Set this to True only after the Arduino firmware has been changed
    # to wait for serial coordinates and print READY.
    ...
    # Map sampled pixel paths to gantry coordinates
    all_sampled_paths_mm, tf_info = map_paths_to_gantry_mm(
        all_sampled_paths_px,
    ...
    print("\nGantry mapping info")
    print(f"  Work area (mm): {tf_info['work_w_mm']} x {tf_info['work_h_mm']}")
    ...
    print(f"  Used size in work area (mm): {tf_info['used_w_mm']:.2f} x {tf_info['used_h_mm']:.2f}")
    print(f"  Offset in work area (mm): x={tf_info['x_offset_mm']:.2f}, y={tf_info['y_offset_mm']:.2f}")

    commands = paths_to_arduino_commands(all_sampled_paths_mm)
    validate_arduino_commands(commands, work_w_mm=work_w_mm, work_h_mm=work_h_mm)
    print(f"\nArduino command count: {len(commands)}")
    ...
    # Save sampled gantry mm coordinates
    coords_mm_out = output_dir / "sampled_points_gantry_mm.txt"
    with open(coords_mm_out, "w", encoding="utf-8") as f:
        f.write("# Sampled contour points mapped to gantry coordinates (x_mm, y_mm)\n")
        f.write(f"# work_w_mm = {work_w_mm}\n")
        f.write(f"# work_h_mm = {work_h_mm}\n")
        ...
        f.write("# Origin convention: top-left of gantry work area is (0,0), +Y is down\n")
    ...
    print(f"Saved sampled point list (gantry mm) to: {coords_mm_out}")

    commands_out = output_dir / "arduino_commands.txt"
    save_arduino_commands(commands, commands_out)
    print(f"Saved Arduino command list to:       {commands_out}")

    gantry_preview_out = output_dir / "gantry_path_preview.png"
    save_gantry_preview(
    ...
    print(f"Saved gantry path preview to:        {gantry_preview_out}")

    if send_to_arduino:
        send_commands_to_arduino(
            commands,
            port=arduino_port,
            baud=9600,
            work_w_mm=work_w_mm,
